...
Antes de implementar esta fase se ha realizado un estudio completo del dataset, que no incluyo por la extensión que implica. En ella se han realizado las siguientes acciones:
...
Existen 4 valores claramente fuera de rango, creo variable nueva actualizándolos con los valores con la mediana según el tipo de construcción
...
pero como el valor está en el conjunto de test no se puede usar para entrenar y el modelo resultante no podrá calcular precios para casas solo de madera, por lo que actualizo el valor a la mediana según la superficie
...
Modifico la fecha de remodelación para casas anteriores a 1950 asignándoles la fecha de construcción
...
*	Métodos wrapper: 
    Evalúan múltiples modelos utilizando procedimientos que agregan y/o eliminan predictores para encontrar la combinación óptima que maximice el rendimiento del modelo, son algoritmos de búsqueda donde los predictores son las entradas y el modelo a optimizar es la salida.
    * Eliminación de características recursivas    
    * Algoritmos genéticos    
    * Simulated annealing
    
*	Métodos de filtro: 
    Analizan la relación que tiene cada predictor con la variable respuesta, evaluando la relevancia de los predictores fuera de los modelos y seleccionando los que pasan algún criterio.
...
# nrounds = 850, max_depth = 2, eta = 0.025, gamma = 0, colsample_bytree =  1, min_child_weight = 1 and subsample = 1.
...
# model were nrounds = 900, max_depth = 2, eta = 0.025, gamma = 0, colsample_bytree = 1, min_child_weight = 3 and subsample = 1
...
# nrounds = 800, max_depth = 2, eta = 0.025, gamma = 0, colsample_bytree = 1, min_child_weight = 8 and subsample = 1.
...
# nrounds = 1350, max_depth = 2, eta = 0.025, gamma = 0, colsample_bytree = 0.4, min_child_weight = 8 and subsample = 0.5.


# RMSE 0.125 Test 0.128
# nrounds = 1950, max_depth = 2, eta = 0.015, gamma = 0, colsample_bytree = 0.4, min_child_weight = 8 and subsample = 0.5.
...
Utilizare la función resamples() para extraer las métricas de los modelos entrenados.
